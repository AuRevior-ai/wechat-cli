...
def search(ctx, keyword, chat, start_time, end_time, limit, offset, fmt, msg_type):
    ...
    try:
        ...
        validate_search_scope(keyword, start_time, end_time)
    ...
    if fmt == 'json':
        ...
        if not paged:
            target = f"包含 \"{keyword}\" 的消息" if keyword else "符合时间范围的消息"
            output(f"在 {scope} 中未找到{target}", 'text')
            return
        if keyword:
            header = f"在 {scope} 中搜索 \"{keyword}\" 找到 {len(paged)} 条结果（offset={offset}, limit={limit}）"
        else:
            header = f"在 {scope} 中按时间范围找到 {len(paged)} 条结果（offset={offset}, limit={limit}）"
