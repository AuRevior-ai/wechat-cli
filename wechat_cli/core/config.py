...
def _detect_db_dir_candidates_windows():
...
def _auto_detect_db_dir_windows():
    return _choose_candidate(_detect_db_dir_candidates_windows())


def _detect_db_dir_candidates_linux():
    seen = set()
    candidates = []
...
def _auto_detect_db_dir_linux():
    return _choose_candidate(_detect_db_dir_candidates_linux())


def _detect_db_dir_candidates_macos():
...
def _auto_detect_db_dir_macos():
    return _choose_candidate(_detect_db_dir_candidates_macos())


def detect_db_dir_candidates():
    """Return all detected WeChat db_storage directories without prompting."""
    if _SYSTEM == "windows":
        return _detect_db_dir_candidates_windows()
    if _SYSTEM == "linux":
        return _detect_db_dir_candidates_linux()
    if _SYSTEM == "darwin":
        return _detect_db_dir_candidates_macos()
    return []
