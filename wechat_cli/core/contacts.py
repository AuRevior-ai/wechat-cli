...
_contact_avatars = None  # {username: avatar_url}
_self_username = None
...
def _load_contacts_from(db_path):
    ...
    try:
        try:
            rows = conn.execute("SELECT username, nick_name, remark, small_head_url, big_head_url FROM contact").fetchall()
        except sqlite3.Error:
            rows = [(uname, nick, remark, '', '') for uname, nick, remark in conn.execute("SELECT username, nick_name, remark FROM contact").fetchall()]
        for r in rows:
            uname, nick, remark, small_url, big_url = r
        ...
        conn.close()
    return names, full, avatars


def get_contact_names(cache, decrypted_dir):
    global _contact_names, _contact_full, _contact_avatars
    ...
    if os.path.exists(pre_decrypted):
        try:
            _contact_names, _contact_full, _contact_avatars = _load_contacts_from(pre_decrypted)
            return _contact_names
        except Exception:
            pass
    ...
    if path:
        try:
            _contact_names, _contact_full, _contact_avatars = _load_contacts_from(path)
            return _contact_names
        except Exception:
            pass
...
def get_contact_avatars(cache, decrypted_dir):
    global _contact_avatars
    if _contact_avatars is None:
        get_contact_names(cache, decrypted_dir)
    return _contact_avatars or {}
