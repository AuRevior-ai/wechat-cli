...
| 6 | 安全与正式交付准备 | **进行中** | B6-G0 complete@`c1d0458`；B6-G1 complete@`bdc98af`；B6-G2 complete@`e0c91df`；B6-G3 complete@`1a07447` + audit `d73cf3f`；B6-G4 staging infrastructure complete，report=`2026-08-14-board-6-staging-infrastructure-gate.md`，Worker Version=`14a19ea3...`；B6-G5 等待独立批准 |
...
- B6-G5：**pending approval / not started**；`admin_principals`/`admin_sessions` 当前均为 0，任何 staging principal provisioning 必须进入 B6-G5 exact mutation matrix
...
**板块 4 已完成；Board 5 accepted complete；Board 6 in progress。** stable Windows E2E、Offline acceptance、beta alignment、fault prepare/register/enable/private-prerelease publish、process-tree repair 后的 fresh rollback re-acceptance、**version-level server suppression**、final fault disable、Cloud Cleanup Gate 和 fresh final verification 均已完成。Worker `rel_board5_bad_052_01` 保持 `enabled=false`、`paused=true`、rollout 100；stable/beta Board 5 license 均已 revoked 且 active devices 0，两条 Board 5 test device row 均 unbound；JD25、050/051、GitHub release/assets/tags、main/NUL 与固定 0.5.0 ZIP 均已 fresh reconcile 无漂移。B6-G0 source integration 与 packaging boundary cleanup 已完成至 `c1d0458`；B6-G1 Update Trust Local Gate 已完成至 `bdc98af`；B6-G2 Admin & Data Security Local Gate 已完成至 `e0c91df`，并通过 fresh Python 524 / 2 expected skips / 0 failures、Worker typecheck 与 Vitest 89/89。B6-G3 Windows Integrity Local Gate 已完成至 `1a07447`，pre-staging security audit=`docs/superpowers/governance/2026-08-13-board-6-pre-staging-security-audit.md` / `d73cf3f`，并通过 fresh Python 607 / 2 expected skips / 0 failures、Worker typecheck 与 Vitest 89/89。B6-G4 Staging Infrastructure Gate 已完成，证据见 `docs/superpowers/governance/2026-08-14-board-6-staging-infrastructure-gate.md`：dedicated staging release R2、D1 migrations `0004`–`0007`、七项 `_V1` staging Secret、Access app/policy/verifier、admin custom domain 与 Worker Version `14a19ea3-5a96-408b-a4e3-0a8d8e4ebe2c` 已 live；workers.dev admin-login bypass probe=403，custom-domain Access probe=302，历史 release rows 未漂移。**下一步唯一待授权 gate 是 B6-G5 Staging Behavior Acceptance Gate**：其 mutation matrix 必须明确列出一个 enabled staging `admin_principals` row 的 provision，以及行为验收实际需要的 disposable licenses/devices/diagnostic objects 和任何 acceptance release Draft/R2/publish/register/enable/cleanup 操作。production、真实 code-signing procurement/use、existing Board 5 releases/JD25、push/merge、local physical cleanup 继续禁止；production D1 placeholder 与空 production routes 继续 fail closed。
