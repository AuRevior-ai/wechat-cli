...
| 7 | 自动化发布与正式上线 | **B7-G7 accepted；B7-G8 Mandatory STOP** | B7-G0..G7 accepted；accepted G7 implementation main=`8afbc7a0...`；exactly one beta canary license/device preserved，0 stable real-user licenses；`0.6.1-canary.1` 已完成 native-immutable publish/register、human enable、真实 update、受控 health-failure rollback、exact suppression、最终 human pause+disable；下一步必须先跨 B7-G8 gate，禁止直接创建首个 real stable user license |
...
**已完成；B6-G0 through B6-G8 accepted complete。当前 closure profile=Private / Controlled Distribution；commercial Authenticode deferred optional。Board 7 已完成并 accepted B7-G0 through B7-G7；accepted G7 implementation main=`8afbc7a074a0cc1cbefa7d9f53da82caa38a9e42`，exact main CI run `31979247562` PASS。G7 evidence=`docs/superpowers/governance/2026-08-17-board-7-production-canary-e2e-acceptance.md`。Production 保留 exactly one active beta canary license/device、0 active stable licenses；stable `0.6.0` enabled/unpaused/rollout 0；internal `0.6.1-canary.1` 已 native-immutable publish、真实 update/rollback/suppression 验收并最终 human-disabled/paused/rollout 100。当前处于 B7-G8 Mandatory STOP；无 real Private user issuance。**
