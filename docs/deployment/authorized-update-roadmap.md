...
- B6-G7：**entered / read-only preflight complete；exact mutation matrix pending explicit approval**。证据=`docs/superpowers/governance/2026-08-14-board-6-key-rotation-preflight.md`。fresh live state：selected mechanisms 全部仍为 V1/key-01，proposed V2 Secret names 尚不存在；D1 dependency inventory 全部 `rows_written=0`；focused Python rotation/admin=35/35，Worker typecheck + Vitest 92/92。计划中的 exact representatives 为 `ADMIN_SESSION_PEPPER_V2`、`DOWNLOAD_TICKET_SECRET_V2`、`CONTACT_ENCRYPTION_KEY_V2`、`lease-key-staging-02`、`release-key-staging-02`；未批准任何 V1 retirement。
