...
| 6 | 安全与正式交付准备 | **进行中** | Board 6 design/plan 已批准；B6-G0 source integration + packaging boundary cleanup 已完成至 `c1d0458`；B6-G1 Update Trust Local Gate 等待独立批准 |
...
### Board 5 → Board 6 risk handoff（已纳入 approved Board 6 design；B6-G0 已完成）
...
- B6-G1：**pending approval / not started**
...
B6-G0 已选择性集成审核后的 Board 5 product lineage，未 wholesale merge Board 5 acceptance tooling；D1 generic packaging dependency 已在 `c1d0458` 通过 `scripts/packaging_paths.py` 收口。Board 6 design/plan 的 approved seed SHA 已冻结并保留；其 canonical tracked copy 已按本轮 governance repair 明确修订 release lifecycle 与 Access/JWT/break-glass contract。Board 5 worktree 中原有两份 untracked Board 6 seed 文档继续保留，未经独立 cleanup authorization 不删除。

### 状态

**进行中；B6-G0 complete，B6-G1 pending approval。**
