...
- B6-G6 Phase A：**provider-neutral readiness repair + provider research complete locally；不等于 B6-G6 real signing complete**。readiness evidence=`docs/superpowers/governance/2026-08-14-board-6-code-signing-entry-readiness.md`；provider decision draft=`docs/superpowers/governance/2026-08-14-board-6-code-signing-provider-decision.md`。`9f4ad0f` 已隔离 Windows PowerShell 5.1 child `PSModulePath`、显式加载 Security module，并用 child-only target env 修复真实 inspector；`e9cb67b` 已扩展 signer issuer/validity/thumbprint 与保守的 timestamp presence/timestamper public evidence contract。fresh Phase A verification：Python 630 / 2 skips / 0 failures，Worker typecheck PASS，Vitest 92/92，signing-focused suite 103/103，真实 Microsoft-signed system probe=`Valid`。real provider/publisher、付款/申请、identity verification、managed/HSM key provisioning、signer installation、concrete adapter、staging publisher policy 与 actual signing 仍 **pending separate explicit approval / not entered**。
...
**进行中；B6-G0 complete，B6-G1 complete，B6-G2 complete，B6-G3 complete，B6-G4 complete，B6-G5 accepted complete；B6-G6 Phase A provider-neutral readiness repair/research complete；real provider procurement/signing pending separate explicit approval；B6-G7/B6-G8 not entered；Board 7 unstarted。**
