...
```bash
python -c "import pathlib,re,sys; files=[pathlib.Path(p) for p in ['AGENTS.md','CHANGELOG.md','README.md','README_CN.md','docs/PROJECT_STATE.md','docs/deployment/authorized-update-roadmap.md','docs/deployment/2026-08-05-local-finalization-report.md','docs/superpowers/plans/2026-08-05-board-4-test-license-and-release.md']]+list(pathlib.Path('docs/superpowers/plans').glob('2026-07-*.md'))+[pathlib.Path('docs/superpowers/plans/2026-08-04-invite-stats-xml-fix.md'),pathlib.Path('docs/superpowers/plans/2026-08-04-wechat-cli-auto-update.md')]; bad=[]; pat=re.compile(r'\[[^\]]*\]\(([^)]+)\)'); [(bad.append((str(f),u)) if not (f.parent/u.split('#',1)[0]).resolve().exists() else None for f in files for u in pat.findall(f.read_text(encoding='utf-8')) if u and not u.startswith(('http://','https://','#','mailto:'))]; print('\n'.join(f'{f}: {u}' for f,u in bad)); sys.exit(bool(bad))"
```
